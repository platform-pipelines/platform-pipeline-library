...
|---|---|---|---|---|
| `overrides` | `Map` | no | `[:]` | Config merged over `.ci/config.yaml`, same shape as the YAML. `configFile` picks a different file. See [configLoad](../config/configLoad.md). |
...
| Lint | always ([lintApp](lintApp.md) skips itself if `lint.enabled: false`) | [lintApp](lintApp.md), then [archiveLintReports](../quality/archiveLintReports.md) |
| Build | always | [buildApp](buildApp.md) |
| Test | always | [testApp](testApp.md), then JUnit results ([appTestReport](appTestReport.md)) and [checkCoverage](../quality/checkCoverage.md) |
| Quality & Security | always | [qualityChecks](../quality/qualityChecks.md) in parallel |
| Package | always | [packageApp](packageApp.md), archive [appArtifacts](appArtifacts.md), [publishArtifact](../deploy/publishArtifact.md) (apps only); if `containerize`: [buildImage](../images/buildImage.md), [generateSbom](../images/generateSbom.md), [signImage](../images/signImage.md) |
| Scan Image | `containerize` and `quality.trivy` | [scanTrivy](../quality/scanTrivy.md) on the pushed image, then [postScanSummary](../quality/postScanSummary.md) |
| Deploy | the branch reaches at least one environment | one `Deploy: <env>` stage per environment from [configEnvironmentsFor](../config/configEnvironmentsFor.md), in order, each running [deployToEnvironment](../deploy/deployToEnvironment.md) |
...
| success | `ci/jenkins` status = success, [notifySlack](../github-slack/notifySlack.md) `SUCCESS` |
...
!!! note
    ...
    [inBuildContainer](../images/inBuildContainer.md). Only the stash — not files
