...
|---|---|---|
| `go` | `test-results.xml` | [goTest](../languages/goTest.md) |
| `python` | `test-results.xml` | [pythonTest](../languages/pythonTest.md) |
| `maven` | `target/surefire-reports/*.xml` | [mavenTest](../languages/mavenTest.md) |
| `gradle` | `build/test-results/test/*.xml` | [gradleTest](../languages/gradleTest.md) |
| `npm` | `junit.xml` | your `npm test` script (e.g. `jest-junit`) |
| `cloudformation` | `checkov-report.xml` | [cfnTest](../infrastructure/cfnTest.md) |
| `terraform`, `docker-only` | `null` | — |

Lint reports are never returned here — they are archived separately by
[archiveLintReports](../quality/archiveLintReports.md).
