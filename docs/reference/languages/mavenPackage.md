...
Nothing. Jars land in `target/`, which [appArtifacts](../pipelines/appArtifacts.md)
(`target/*.jar`) archives and [publishArtifact](../deploy/publishArtifact.md) uploads.
...
Called by [packageApp](../pipelines/packageApp.md) when `cfg.buildTool == 'maven'`.
