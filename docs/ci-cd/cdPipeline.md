...
|---|---|---|---|---|
| `overrides` | `Map` | no | `[:]` | Config merged over `.ci/config.yaml`. See [configLoad](../other/configLoad.md). |
...
| `imageRepo` | `ghcr.io/acme/inventory-api` | [imageDigest](imageDigest.md), deploy |
| `environments[].promoteFrom` | `staging` | [cdResolveImage](cdResolveImage.md) |
| `environments[].requiresApproval` / `approvers` | `true` / `[platform-leads]` | [approvalGate](approvalGate.md) |
...
|---|---|---|
| Resolve | always | checkout, [configLoad](../other/configLoad.md), [cdResolveImage](cdResolveImage.md), [withRegistryAuth](withRegistryAuth.md) + [imageDigest](imageDigest.md); build named `#12 prod ← 1.4.0` |
| Deploy | not `DRY_RUN` | [deployToEnvironment](../cloud/deployToEnvironment.md) (approval, then GitOps bump + Argo sync, or [deployEcs](../cloud/deployEcs.md)) |
