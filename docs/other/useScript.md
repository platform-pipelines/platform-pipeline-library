...
|---|---|---|---|
| `coverage_percent.py` | `<report>` | `82.4` | [coveragePercent](../ci-cd/coveragePercent.md) |
| `trivy_summary.py` | `<trivy.json>` | `CRITICAL=1, HIGH=3` or `clean` | [trivySummary](../ci-cd/trivySummary.md) |
| `sbom_summary.py` | `<sbom.cdx.json>` | `214 components (pypi=180, deb=34)` | [generateSbom](../ci-cd/generateSbom.md) |
| `sonar_gate.py` | `<ceTaskUrl> <host> <token>` | `OK`, `ERROR`, `WARN` or `PENDING` | [sonarWaitForGate](../ci-cd/sonarWaitForGate.md) |
| `terraform_plan_summary.py` | `<plan.json>` | `1 to create, 1 to delete \| DESTRUCTIVE: …` | [terraformPlan](../cloud/terraformPlan.md) |
| `cfn_changeset_summary.py` | `<changeset.json>` | `2 to add, 1 to modify` / `no changes` / `failed: …` | [cfnChangeSet](../cloud/cfnChangeSet.md) |
| `eslint_to_junit.py` | stdin: ESLint JSON | JUnit XML | [nodeLint](../ci-cd/nodeLint.md) |
| `github_file_sha.py` | stdin: contents API JSON | blob sha or empty | [githubFileSha](githubFileSha.md) |
| `find_pr_comment.py` | `<marker>`; stdin: comments JSON | comment id or empty | [githubFindComment](githubFindComment.md) |
